 ...
 *
 * Provides rich metadata: titles, plots, ratings, cast with photos,
 * trailers, similar titles, and HD images from TMDB's CDN.
 *
  ...
  (error) => {
    ...
    if (status === 401) {
      import('../store/useToastStore').then((m) =>
        m.default.getState().addToast('Invalid TMDB API key. Check your settings.', 'error', 6000)
      )
    }
    throw error
  }
